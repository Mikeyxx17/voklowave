...
一个轻量级的实时团队聊天应用，类似 Slack / Discord。支持多频道、实时消息推送、用户认证、邮箱验证和访客模式。
...
- [lettre](https://github.com/lettre/lettre) — SMTP 邮件发送（邮箱验证码）
...
│   └── src/
│       ├── main.rs              # 入口：路由注册、CORS、清理任务、启动监听 :3000
...
| `SMTP_SERVER` | 否 | `smtp.office365.com` | SMTP 邮件服务器地址 |
| `SMTP_USERNAME` | 否 | — | SMTP 登录邮箱 |
| `SMTP_PASSWORD` | 否 | — | SMTP 登录密码或应用专用密码 |
| `ALLOWED_DOMAINS` | 否 | — | 允许注册的邮箱域名，逗号分隔（如 `gmail.com,outlook.com`）。留空则拒绝所有注册 |
